...
- `2026-04-09`: All tasks T1-T9 DONE. Full documentation review completed: README updated with raw capture, enhanced PIDs, O2 monitoring, v2 recording format, and `c` keybinding. MANUAL updated with new Section 16 (Raw Protocol Capture), corrected connection.json filename, expanded recording data types, and renumbered sections. Module-level doc comments added to analysis, ai, tui, widget, and recording modules. Test suite expanded with coverage for VehicleData, DrivingBehavior, FuelEconomyState, and replay controller. See `docs/OUTSTANDING.md` for follow-up items.
